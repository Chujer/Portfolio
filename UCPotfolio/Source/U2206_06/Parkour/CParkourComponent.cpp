...
void UCParkourComponent::CheckTrace_Center()
{
	...
	HitObstacle = HitResults->GetActor();
	...
	float z = FMath::Abs(minBound.Z - maxBound.Z);		//부피구하기
	HitObstacleExtent = FVector(x, y, z);

	HitDistance = hitResult.Distance;		//거리

	ToFrontYaw = UKismetMathLibrary::MakeRotFromX(-hitResult.ImpactNormal).Yaw;	//임펙트 위치의 노말값의 반대편
...
{
	CheckFalse(OwnerCharacter->GetCharacterMovement()->IsFalling());	//플레이어가 낙하중(isfalling)이 아니면 return

	CheckTrue(bFalling); // 이미 낙하 라고 설정되었다면 더 검사하지 않음
	bFalling = true;
...
void UCParkourComponent::DoParkour(bool bLanded = false)
{
...
{
	CheckFalseResult(HitResults[(int32)EParkourArrowType::Ceil].bBlockingHit, false);	//머리위 화살표가 충돌이면 true반환 아님return
