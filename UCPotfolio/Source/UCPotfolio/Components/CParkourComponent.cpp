...
UCParkourComponent::UCParkourComponent()
{
	...
	//CHelpers::GetAsset<UDataTable>(&DataTable, "");
}
...
void UCParkourComponent::BeginPlay()
{
	...
	arrow->GetChildrenComponents(false, components);	//ArrowGroup의 자식 Arrow들을 가져온다.
...
void UCParkourComponent::LineTrace(EParkourArrowType InType)
{
	...
	UKismetSystemLibrary::LineTraceSingle(GetWorld(), start, end, ETraceTypeQuery::TraceTypeQuery3, false, ignores, 
		DebugType, HitResults[(int32)InType], true, color, FLinearColor::White);
...
void UCParkourComponent::CheckTrace_Center()
{
	...
	HitObstacleExtent = FVector(x, y, z);	//파쿠르 하려는 객체의 크기(스테틱매쉬의 크기)
...
void UCParkourComponent::CheckTrace_Land()
{
	...
	CheckTrue(bFalling);	//첫 낙하 타이밍에만 사용하기 위해 검사
